...
def grid_id2pos_3d_batch(pos_np: np.ndarray, camera_height: float, cs: float, gs: int):
    ...
    return np.concatenate(
        [cam_x.reshape((-1, 1)), cam_y.reshape((-1, 1)), cam_z.reshape((-1, 1))], axis=1
    )
...
def save_clip_sparse_map(
    save_path: str, clip_sparse_map: np.ndarray, robot_pose_list: List[np.ndarray]
):
...
def grid_id2pos_3d_real_batch(pos_np: np.ndarray, camera_height: float, cs: float, gs: int):
    ...
    return np.concatenate(
        [base_x.reshape((-1, 1)), base_y.reshape((-1, 1)), base_z.reshape((-1, 1))], axis=1
    )