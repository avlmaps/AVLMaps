...
from avlmaps.robot.lang_robot import LangRobot
from avlmaps.dataloader.habitat_dataloader import VLMapsDataloaderHabitat
from avlmaps.navigator.navigator import Navigator
from avlmaps.controller.discrete_nav_controller import DiscreteNavController
...
from avlmaps.utils.habitat_utils import make_cfg, tf2agent_state, agent_state2tf, display_sample
from avlmaps.utils.matterport3d_categories import mp3dcat
...
class HabitatLanguageRobot(LangRobot):
    ...
    def __init__(self, config: DictConfig):
        ...
        data_dir = Path(self.config["data_paths"]["avlmaps_data_dir"]) / "vlmaps_dataset"
    ...
    def setup_scene(self, scene_id: int):
        ...
        avlmaps_data_dir = self.vlmaps_data_save_dirs[scene_id]
        print(avlmaps_data_dir)
        self.scene_name = avlmaps_data_dir.name.split("_")[0]

        self._setup_sim(self.scene_name)

        self.setup_map(avlmaps_data_dir)
        ...
        # self._setup_localizer(avlmaps_data_dir)

    def setup_map(self, avlmaps_data_dir: str):
        self.load_scene_map(avlmaps_data_dir, self.config["map_config"])
        ...
        self.vlmaps_dataloader = VLMapsDataloaderHabitat(avlmaps_data_dir, self.config.map_config, map=self.map)
...
def main(config: DictConfig) -> None:
    data_dir = Path(config.data_paths.avlmaps_data_dir) / "vlmaps_dataset"
