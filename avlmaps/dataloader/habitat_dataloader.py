...
from avlmaps.utils.time_utils import Tic
from avlmaps.utils.mapping_utils import cvt_pose_vec2tf, base_pos2grid_id_3d, grid_id2base_pos_3d, base_rot_mat2theta
...
class VLMapsDataloaderHabitat:
    ...
    def __init__(
        ...
        self.rmax = self.map.rmax
        self.cmin = self.map.cmin
        self.cmax = self.map.cmax
    ...
    def get_color_topdown_bgr_cropped(self, times: float = 1) -> np.array:
        ...
        color_top_down = color_top_down[self.rmin : self.rmax + 1, self.cmin : self.cmax + 1]
