...
import avlmaps.audioclip.utils.transforms as transforms
...
class UrbanSound8K(td.Dataset):
    ...
    def _load_worker(fn: str, path_to_file: str, sample_rate: int, mono: bool = False) -> Tuple[str, int, np.ndarray]:
        wav, sample_rate_ = sf.read(path_to_file, dtype="float32", always_2d=True)
    ...
    def load_data(self):
        # read metadata
        meta = pd.read_csv(
            os.path.join(self.root, "metadata", "UrbanSound8K.csv"), sep=",", index_col="slice_file_name"
        )
        ...
        if self.random_split_seed is not None:
            ...
            for fold_idx, (train_ids, test_ids) in enumerate(
                skf.split(np.zeros(len(meta)), meta["classID"].values.astype(int)), 1
            ):
