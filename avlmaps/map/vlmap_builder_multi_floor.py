...
from avlmaps.utils.lseg_utils import get_lseg_feat
...
from avlmaps.lseg.modules.models.lseg_net import LSegEncNet
...
class VLMapBuilderMultiFloor:
    ...
    def create_global_map(self):
        ...
        The global frame: y upward, x right, z backward. If traditional camera coordinate system is used for backprojecting
        pixels to 3D. A 180 deg rotation around x axis T_rotx needs to be apply before mapping those points to the
        global frame. P_global = pose @ T_rotx @ P_camera.
        """
        # access config info
        camera_height = self.map_config.pose_info.camera_height
        building_init_height = self.map_config.pose_info.building_init_height
        cs = self.map_config.cell_size
        gs = self.map_config.grid_size
        depth_sample_rate = self.map_config.depth_sample_rate
        self.camera_pose_tfs = [np.loadtxt(path_i).reshape((4, 4)) for path_i in self.pose_paths]
        self.init_cam_tf = self.camera_pose_tfs[0]
        self.inv_init_cam_tf = np.linalg.inv(self.init_cam_tf)

        self.habitat2cam_rot_tf = np.eye(4)
        self.habitat2cam_rot_tf[1, 1] = -1
        self.habitat2cam_rot_tf[2, 2] = -1

        self.map_save_dir = self.data_dir / "vlmap_multi_floor"
        os.makedirs(self.map_save_dir, exist_ok=True)
        self.map_save_path = self.map_save_dir / "vlmaps_multi_floor.h5df"
        ...
        for frame_i, (rgb_path, depth_path, camera_pose_tf) in enumerate(pbar):
            ...
            depth = load_depth_img(depth_path.as_posix()) / 1000.0
        ...
        for frame_i, (rgb_path, depth_path, camera_pose_tf) in enumerate(pbar):
            ...
            for i, (p, p_local) in enumerate(zip(pc_global.T, pc.T)):
                ...
                if row >= height_map.shape[0] or col >= height_map.shape[1]:
                    print("out of range")
                    continue
    ...
    def _init_map(self, pcd_min: np.ndarray, pcd_max: np.ndarray, cs: float, map_path: Path) -> Tuple:
        ...
        if os.path.exists(map_path):
            (mapped_iter_list, grid_feat, grid_pos, weight, occupied_ids, grid_rgb, pcd_min, cs) = self.load_3d_map(
                self.map_save_path
            )
