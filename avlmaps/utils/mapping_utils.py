...
def save_3d_map(
    ...
    with h5py.File(save_path, "w") as f:
        ...
        if init_height_id is not None:
            f.create_dataset("init_height_id", data=np.array(init_height_id, dtype=np.int32))
...
def load_3d_map(
    map_path: str,
) -> Tuple[Set[int], np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, Optional[float]]:
    ...
    with h5py.File(map_path, "r") as f:
        ...
        if "grid_rgb" in f:
            grid_rgb = f["grid_rgb"][:]
        if "init_height_id" in f:
            init_height_id = f["init_height_id"]
            return mapped_iter_list, grid_feat, grid_pos, weight, occupied_ids, grid_rgb, init_height_id
