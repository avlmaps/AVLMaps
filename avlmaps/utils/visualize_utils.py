...
def visualize_rgb_map_3d(pc: np.ndarray, rgb: np.ndarray, save_path: Union[str, Path] = None):
    grid_rgb = rgb / 255.0
...
def visualize_masked_map_3d(
    pc: np.ndarray, mask: np.ndarray, rgb: np.ndarray, transparency: float = 0.5, save_path: Union[str, Path] = None
):
    heatmap = mask.astype(np.float16)
    visualize_heatmap_3d(pc, heatmap, rgb, transparency, save_path)


def convert_heatmap_to_rgb(heatmap: np.ndarray, rgb: np.ndarray, transparency: float = 0.5) -> np.ndarray:
...
def visualize_heatmap_3d(
    pc: np.ndarray, heatmap: np.ndarray, rgb: np.ndarray, transparency: float = 0.5, save_path: Union[str, Path] = None
):
    print("heatmap min: ", np.min(heatmap))
    print("heatmap max: ", np.max(heatmap))
    print("heatmap unique: ", np.unique(heatmap))
    heat_rgb = convert_heatmap_to_rgb(heatmap, rgb, transparency)
