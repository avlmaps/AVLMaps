...
class HLocLocalizer:
    ...
    def init_depth_with_depth_folder(
        self, depths_dir, frame_sample_interval=50, key=lambda x: int(x.split(".")[0].split("_")[-1]), show=False, extension=".npy"
    ):
    ...
    def init_pose_with_pose_folder(self, poses_dir, frame_sample_interval=50, key=lambda x: int(x.split(".")[0].split("_")[-1])):
        self.frame_sample_interval = frame_sample_interval
    ...
    @torch.no_grad()
    def match_keypoints_with_detection_output(
        self, kpts_dict0, kpts_dict1, image0_tensor_shape, image1_tensor_shape
    ):
        """
        Input:
    ...
    def match_keypoints(self, img0, img1, vis: bool = False):
        ...
        if vis:
            self.visulize_keypoints(
                img0, img1, kpts0, kpts1, mkpts0, mkpts1, confidence, valid
            )
        return mkpts0, mkpts1, confidence

    def visulize_keypoints(
        self, img0, img1, kpts0, kpts1, mkpts0, mkpts1, confidence, valid
    ):
    ...
    def compute_global_descriptor(
        ...
        if reference:
            features_path = os.path.join(
                self.features_dir,
                self.descriptor_file_prefix + "reference_features.txt",
            )
            self.ref_features_path = features_path
            print(f"reading reference global features at {features_path}")
        else:
            features_path = os.path.join(
                self.features_dir, self.descriptor_file_prefix + "query_features.txt"
            )
    ...
    def localize_agent(self, rgb_obs):
        ...
        reference_features_path = os.path.join(
            self.features_dir, self.descriptor_file_prefix + "reference_features.txt"
        )
        ...
        query_desc = pairs_from_retrieval.get_descriptors(
            ["000000"], query_features_path
        )
    ...
    def _get_relative_pose_with_depth(
        ...
        mkpts0, mkpts1, confidence = self.match_keypoints(
            rgb_ref, rgb_obs, vis=vis
        )  # mkpts0 (N, 2)
        ...
        mkpts1_list = [x.reshape((2, 1)) for x in mkpts1]
        mkpts0_3d_list = [x.reshape((3, 1)) for x in mkpts0_3d.T]
    ...
    def localize_agent_with_depth(
        ...
        if self.depth_extension == ".npy":
            ref_depth = load_depth(ref_depth_path).astype(float)
...
def get_frames_tfs(
    ...
    for frame_i, frame in enumerate(segment_frames):
        ...
        if masked_obst is not None:
            ...
            masked_obst = cv2.circle(
                masked_obst, (int(refcol), int(refrow)), 2, (0, 0, 255), -1
            )  # red (BGR)
            masked_obst = cv2.circle(
                masked_obst, (int(col), int(row)), 2, (255, 0, 0), -1
            )  # blue (BGR)
